...
!(function(global) {                                                                                               // 11
  ...
  GeneratorFunction.displayName = "GeneratorFunction";                                                             // 87
                                                                                                                   // 88
  // Helper for defining the .next, .throw, and .return methods of the                                             // 89
  // Iterator interface in terms of a single ._invoke method.                                                      // 90
  function defineIteratorMethods(prototype) {                                                                      // 91
    ["next", "throw", "return"].forEach(function(method) {                                                         // 92
      prototype[method] = function(arg) {                                                                          // 93
        return this._invoke(method, arg);                                                                          // 94
      };                                                                                                           // 95
    });                                                                                                            // 96
  }                                                                                                                // 97
                                                                                                                   // 98
  runtime.isGeneratorFunction = function(genFun) {                                                                 // 99
    var ctor = typeof genFun === "function" && genFun.constructor;                                                 // 100
    return ctor                                                                                                    // 101
      ? ctor === GeneratorFunction ||                                                                              // 102
        // For the native GeneratorFunction constructor, the best we can                                           // 103
        // do is to check its .name property.                                                                      // 104
        (ctor.displayName || ctor.name) === "GeneratorFunction"                                                    // 105
      : false;                                                                                                     // 106
  };                                                                                                               // 107
                                                                                                                   // 108
  runtime.mark = function(genFun) {                                                                                // 109
    genFun.__proto__ = GeneratorFunctionPrototype;                                                                 // 110
    genFun.prototype = Object.create(Gp);                                                                          // 111
    return genFun;                                                                                                 // 112
  };                                                                                                               // 113
                                                                                                                   // 114
  // Within the body of any async function, `await x` is transformed to                                            // 115
  // `yield regeneratorRuntime.awrap(x)`, so that the runtime can test                                             // 116
  // `value instanceof AwaitArgument` to determine if the yielded value is                                         // 117
  // meant to be awaited. Some may consider the name of this method too                                            // 118
  // cutesy, but they are curmudgeons.                                                                             // 119
  runtime.awrap = function(arg) {                                                                                  // 120
    return new AwaitArgument(arg);                                                                                 // 121
  };                                                                                                               // 122
                                                                                                                   // 123
  function AwaitArgument(arg) {                                                                                    // 124
    this.arg = arg;                                                                                                // 125
  }                                                                                                                // 126
                                                                                                                   // 127
  function AsyncIterator(generator) {                                                                              // 128
    // This invoke function is written in a style that assumes some                                                // 129
    // calling function (or Promise) will handle exceptions.                                                       // 130
    function invoke(method, arg) {                                                                                 // 131
      var result = generator[method](arg);                                                                         // 132
      var value = result.value;                                                                                    // 133
      return value instanceof AwaitArgument                                                                        // 134
        ? Promise.resolve(value.arg).then(invokeNext, invokeThrow)                                                 // 135
        : Promise.resolve(value).then(function(unwrapped) {                                                        // 136
            // When a yielded Promise is resolved, its final value becomes                                         // 137
            // the .value of the Promise<{value,done}> result for the                                              // 138
            // current iteration. If the Promise is rejected, however, the                                         // 139
            // result for this iteration will be rejected with the same                                            // 140
            // reason. Note that rejections of yielded Promises are not                                            // 141
            // thrown back into the generator function, as is the case                                             // 142
            // when an awaited Promise is rejected. This difference in                                             // 143
            // behavior between yield and await is important, because it                                           // 144
            // allows the consumer to decide what to do with the yielded                                           // 145
            // rejection (swallow it and continue, manually .throw it back                                         // 146
            // into the generator, abandon iteration, whatever). With                                              // 147
            // await, by contrast, there is no opportunity to examine the                                          // 148
            // rejection reason outside the generator function, so the                                             // 149
            // only option is to throw it from the await expression, and                                           // 150
            // let the generator function handle the exception.                                                    // 151
            result.value = unwrapped;                                                                              // 152
            return result;                                                                                         // 153
          });                                                                                                      // 154
    }                                                                                                              // 155
                                                                                                                   // 156
    if (typeof process === "object" && process.domain) {                                                           // 157
      invoke = process.domain.bind(invoke);                                                                        // 158
    }                                                                                                              // 159
                                                                                                                   // 160
    var invokeNext = invoke.bind(generator, "next");                                                               // 161
    var invokeThrow = invoke.bind(generator, "throw");                                                             // 162
    var invokeReturn = invoke.bind(generator, "return");                                                           // 163
    var previousPromise;                                                                                           // 164
                                                                                                                   // 165
    function enqueue(method, arg) {                                                                                // 166
      var enqueueResult =                                                                                          // 167
        // If enqueue has been called before, then we want to wait until                                           // 168
        // all previous Promises have been resolved before calling invoke,                                         // 169
        // so that results are always delivered in the correct order. If                                           // 170
        // enqueue has not been called before, then it is important to                                             // 171
        // call invoke immediately, without waiting on a callback to fire,                                         // 172
        // so that the async generator function has the opportunity to do                                          // 173
        // any necessary setup in a predictable way. This predictability                                           // 174
        // is why the Promise constructor synchronously invokes its                                                // 175
        // executor callback, and why async functions synchronously                                                // 176
        // execute code before the first await. Since we implement simple                                          // 177
        // async functions in terms of async generators, it is especially                                          // 178
        // important to get this right, even though it requires care.                                              // 179
        previousPromise ? previousPromise.then(function() {                                                        // 180
          return invoke(method, arg);                                                                              // 181
        }) : new Promise(function(resolve) {                                                                       // 182
          resolve(invoke(method, arg));                                                                            // 183
        });                                                                                                        // 184
                                                                                                                   // 185
      // Avoid propagating enqueueResult failures to Promises returned by                                          // 186
      // later invocations of the iterator.                                                                        // 187
      previousPromise = enqueueResult["catch"](function(ignored){});                                               // 188
                                                                                                                   // 189
      return enqueueResult;                                                                                        // 190
    }                                                                                                              // 191
                                                                                                                   // 192
    // Define the unified helper method that is used to implement .next,                                           // 193
    // .throw, and .return (see defineIteratorMethods).                                                            // 194
    this._invoke = enqueue;                                                                                        // 195
  }                                                                                                                // 196
                                                                                                                   // 197
  defineIteratorMethods(AsyncIterator.prototype);                                                                  // 198
                                                                                                                   // 199
  // Note that simple async functions are implemented on top of                                                    // 200
  // AsyncIterator objects; they just return a Promise for the value of                                            // 201
  // the final result produced by the iterator.                                                                    // 202
  runtime.async = function(innerFn, outerFn, self, tryLocsList) {                                                  // 203
    var iter = new AsyncIterator(                                                                                  // 204
      wrap(innerFn, outerFn, self, tryLocsList)                                                                    // 205
    );                                                                                                             // 206
                                                                                                                   // 207
    return runtime.isGeneratorFunction(outerFn)                                                                    // 208
      ? iter // If outerFn is a generator, return the full iterator.                                               // 209
      : iter.next().then(function(result) {                                                                        // 210
          return result.done ? result.value : iter.next();                                                         // 211
        });                                                                                                        // 212
  };                                                                                                               // 213
                                                                                                                   // 214
  function makeInvokeMethod(innerFn, self, context) {                                                              // 215
    var state = GenStateSuspendedStart;                                                                            // 216
                                                                                                                   // 217
    return function invoke(method, arg) {                                                                          // 218
      if (state === GenStateExecuting) {                                                                           // 219
        throw new Error("Generator is already running");                                                           // 220
      }                                                                                                            // 221
                                                                                                                   // 222
      if (state === GenStateCompleted) {                                                                           // 223
        if (method === "throw") {                                                                                  // 224
          throw arg;                                                                                               // 225
        }                                                                                                          // 226
                                                                                                                   // 227
        // Be forgiving, per 25.3.3.3.3 of the spec:                                                               // 228
        // https://people.mozilla.org/~jorendorff/es6-draft.html#sec-generatorresume                               // 229
        return doneResult();                                                                                       // 230
      }                                                                                                            // 231
                                                                                                                   // 232
      while (true) {                                                                                               // 233
        var delegate = context.delegate;                                                                           // 234
        if (delegate) {                                                                                            // 235
          if (method === "return" ||                                                                               // 236
              (method === "throw" && delegate.iterator[method] === undefined)) {                                   // 237
            // A return or throw (when the delegate iterator has no throw                                          // 238
            // method) always terminates the yield* loop.                                                          // 239
            context.delegate = null;                                                                               // 240
                                                                                                                   // 241
            // If the delegate iterator has a return method, give it a                                             // 242
            // chance to clean up.                                                                                 // 243
            var returnMethod = delegate.iterator["return"];                                                        // 244
            if (returnMethod) {                                                                                    // 245
              var record = tryCatch(returnMethod, delegate.iterator, arg);                                         // 246
              if (record.type === "throw") {                                                                       // 247
                // If the return method threw an exception, let that                                               // 248
                // exception prevail over the original return or throw.                                            // 249
                method = "throw";                                                                                  // 250
                arg = record.arg;                                                                                  // 251
                continue;                                                                                          // 252
              }                                                                                                    // 253
                                                                                                                   ...
                                                                                                                   // 255
            if (method === "return") {                                                                             // 256
              // Continue with the outer return, now that the delegate                                             // 257
              // iterator has been terminated.                                                                     // 258
              continue;                                                                                            // 259
            }                                                                                                      // 260
          }                                                                                                        // 261
                                                                                                                   // 262
          var record = tryCatch(                                                                                   // 263
            delegate.iterator[method],                                                                             // 264
            delegate.iterator,                                                                                     // 265
            arg                                                                                                    // 266
          );                                                                                                       // 267
                                                                                                                   // 268
          if (record.type === "throw") {                                                                           // 269
            context.delegate = null;                                                                               // 270
                                                                                                                   // 271
            // Like returning generator.throw(uncaught), but without the                                           // 272
            // overhead of an extra function call.                                                                 // 273
            method = "throw";                                                                                      // 274
            arg = record.arg;                                                                                      // 275
            continue;                                                                                              // 276
          }                                                                                                        // 277
                                                                                                                   // 278
          // Delegate generator ran and handled its own exceptions so                                              // 279
          // regardless of what the method was, we continue as if it is                                            // 280
          // "next" with an undefined arg.                                                                         // 281
          method = "next";                                                                                         // 282
          arg = undefined;                                                                                         // 283
                                                                                                                   // 284
          var info = record.arg;                                                                                   // 285
          if (info.done) {                                                                                         // 286
            context[delegate.resultName] = info.value;                                                             // 287
            context.next = delegate.nextLoc;                                                                       // 288
          } else {                                                                                                 // 289
            state = GenStateSuspendedYield;                                                                        // 290
            return info;                                                                                           // 291
          }                                                                                                        // 292
                                                                                                                   // 293
          context.delegate = null;                                                                                 // 294
        }                                                                                                          // 295
                                                                                                                   // 296
        if (method === "next") {                                                                                   // 297
          if (state === GenStateSuspendedYield) {                                                                  // 298
            context.sent = arg;                                                                                    // 299
          } else {                                                                                                 // 300
            context.sent = undefined;                                                                              // 301
          }                                                                                                        // 302
                                                                                                                   // 303
        } else if (method === "throw") {                                                                           // 304
          if (state === GenStateSuspendedStart) {                                                                  // 305
            state = GenStateCompleted;                                                                             // 306
            throw arg;                                                                                             // 307
          }                                                                                                        // 308
                                                                                                                   // 309
          if (context.dispatchException(arg)) {                                                                    // 310
            // If the dispatched exception was caught by a catch block,                                            // 311
            // then let that catch block handle the exception normally.                                            // 312
            method = "next";                                                                                       // 313
            arg = undefined;                                                                                       // 314
          }                                                                                                        // 315
                                                                                                                   // 316
        } else if (method === "return") {                                                                          // 317
          context.abrupt("return", arg);                                                                           // 318
        }                                                                                                          // 319
                                                                                                                   // 320
        state = GenStateExecuting;                                                                                 // 321
                                                                                                                   // 322
        var record = tryCatch(innerFn, self, context);                                                             // 323
        if (record.type === "normal") {                                                                            // 324
          // If an exception is thrown from innerFn, we leave state ===                                            // 325
          // GenStateExecuting and loop back for another invocation.                                               // 326
          state = context.done                                                                                     // 327
            ? GenStateCompleted                                                                                    // 328
            : GenStateSuspendedYield;                                                                              // 329
                                                                                                                   // 330
          var info = {                                                                                             // 331
            value: record.arg,                                                                                     // 332
            done: context.done                                                                                     // 333
          };                                                                                                       // 334
                                                                                                                   // 335
          if (record.arg === ContinueSentinel) {                                                                   // 336
            if (context.delegate && method === "next") {                                                           // 337
              // Deliberately forget the last sent value so that we don't                                          // 338
              // accidentally pass it on to the delegate.                                                          // 339
              arg = undefined;                                                                                     // 340
            }                                                                                                      // 341
          } else {                                                                                                 // 342
            return info;                                                                                           // 343
                                                                                                                   ...
                                                                                                                   // 345
        } else if (record.type === "throw") {                                                                      // 346
          state = GenStateCompleted;                                                                               // 347
          // Dispatch the exception by looping back around to the                                                  // 348
          // context.dispatchException(arg) call above.                                                            // 349
          method = "throw";                                                                                        // 350
          arg = record.arg;                                                                                        // 351
        }                                                                                                          // 352
      }                                                                                                            // 353
                                                                                                                   ...
                                                                                                                   // 356
  // Define Generator.prototype.{next,throw,return} in terms of the                                                // 357
  // unified ._invoke helper method.                                                                               // 358
  defineIteratorMethods(Gp);                                                                                       // 359
                                                                                                                   // 360
  Gp[iteratorSymbol] = function() {                                                                                // 361
    return this;                                                                                                   // 362
  };                                                                                                               // 363
                                                                                                                   // 364
  Gp.toString = function() {                                                                                       // 365
    return "[object Generator]";                                                                                   // 366
  };                                                                                                               // 367
                                                                                                                   // 368
  function pushTryEntry(locs) {                                                                                    // 369
    var entry = { tryLoc: locs[0] };                                                                               // 370
                                                                                                                   // 371
    if (1 in locs) {                                                                                               // 372
      entry.catchLoc = locs[1];                                                                                    // 373
    }                                                                                                              // 374
                                                                                                                   // 375
    if (2 in locs) {                                                                                               // 376
      entry.finallyLoc = locs[2];                                                                                  // 377
      entry.afterLoc = locs[3];                                                                                    // 378
    }                                                                                                              // 379
                                                                                                                   // 380
    this.tryEntries.push(entry);                                                                                   // 381
  }                                                                                                                // 382
                                                                                                                   // 383
  function resetTryEntry(entry) {                                                                                  // 384
    var record = entry.completion || {};                                                                           // 385
    record.type = "normal";                                                                                        // 386
    delete record.arg;                                                                                             // 387
    entry.completion = record;                                                                                     // 388
  }                                                                                                                // 389
                                                                                                                   // 390
  function Context(tryLocsList) {                                                                                  // 391
    // The root entry object (effectively a try statement without a catch                                          // 392
    // or a finally block) gives us a place to store values thrown from                                            // 393
    // locations where there is no enclosing try statement.                                                        // 394
    this.tryEntries = [{ tryLoc: "root" }];                                                                        // 395
    tryLocsList.forEach(pushTryEntry, this);                                                                       // 396
    this.reset(true);                                                                                              // 397
  }                                                                                                                // 398
                                                                                                                   // 399
  runtime.keys = function(object) {                                                                                // 400
    var keys = [];                                                                                                 // 401
    for (var key in object) {                                                                                      // 402
      keys.push(key);                                                                                              // 403
    }                                                                                                              // 404
    keys.reverse();                                                                                                // 405
                                                                                                                   // 406
    // Rather than returning an object with a next method, we keep                                                 // 407
    // things simple and return the next function itself.                                                          // 408
    return function next() {                                                                                       // 409
      while (keys.length) {                                                                                        // 410
        var key = keys.pop();                                                                                      // 411
        if (key in object) {                                                                                       // 412
          next.value = key;                                                                                        // 413
          next.done = false;                                                                                       // 414
          return next;                                                                                             // 415
        }                                                                                                          // 416
                                                                                                                   ...
                                                                                                                   // 418
      // To avoid creating an additional object, we just hang the .value                                           // 419
      // and .done properties off the next function object itself. This                                            // 420
      // also ensures that the minifier will not anonymize the function.                                           // 421
      next.done = true;                                                                                            // 422
      return next;                                                                                                 // 423
    };                                                                                                             // 424
  };                                                                                                               // 425
                                                                                                                   // 426
  function values(iterable) {                                                                                      // 427
    if (iterable) {                                                                                                // 428
      var iteratorMethod = iterable[iteratorSymbol];                                                               // 429
      if (iteratorMethod) {                                                                                        // 430
        return iteratorMethod.call(iterable);                                                                      // 431
      }                                                                                                            // 432
                                                                                                                   // 433
      if (typeof iterable.next === "function") {                                                                   // 434
        return iterable;                                                                                           // 435
      }                                                                                                            // 436
                                                                                                                   // 437
      if (!isNaN(iterable.length)) {                                                                               // 438
        var i = -1, next = function next() {                                                                       // 439
          while (++i < iterable.length) {                                                                          // 440
            if (hasOwn.call(iterable, i)) {                                                                        // 441
              next.value = iterable[i];                                                                            // 442
              next.done = false;                                                                                   // 443
              return next;                                                                                         // 444
            }                                                                                                      // 445
          }                                                                                                        // 446
                                                                                                                   // 447
          next.value = undefined;                                                                                  // 448
          next.done = true;                                                                                        // 449
                                                                                                                   // 450
          return next;                                                                                             // 451
        };                                                                                                         // 452
                                                                                                                   // 453
        return next.next = next;                                                                                   // 454
      }                                                                                                            // 455
    }                                                                                                              // 456
                                                                                                                   // 457
    // Return an iterator with no values.                                                                          // 458
    return { next: doneResult };                                                                                   // 459
  }                                                                                                                // 460
  runtime.values = values;                                                                                         // 461
                                                                                                                   // 462
  function doneResult() {                                                                                          // 463
    return { value: undefined, done: true };                                                                       // 464
  }                                                                                                                // 465
                                                                                                                   // 466
  Context.prototype = {                                                                                            // 467
    constructor: Context,                                                                                          // 468
                                                                                                                   // 469
    reset: function(skipTempReset) {                                                                               // 470
      this.prev = 0;                                                                                               // 471
      this.next = 0;                                                                                               // 472
      this.sent = undefined;                                                                                       // 473
      this.done = false;                                                                                           // 474
      this.delegate = null;                                                                                        // 475
                                                                                                                   // 476
      this.tryEntries.forEach(resetTryEntry);                                                                      // 477
                                                                                                                   // 478
      if (!skipTempReset) {                                                                                        // 479
        for (var name in this) {                                                                                   // 480
          // Not sure about the optimal order of these conditions:                                                 // 481
          if (name.charAt(0) === "t" &&                                                                            // 482
              hasOwn.call(this, name) &&                                                                           // 483
              !isNaN(+name.slice(1))) {                                                                            // 484
            this[name] = undefined;                                                                                // 485
          }                                                                                                        // 486
        }                                                                                                          // 487
      }                                                                                                            // 488
    },                                                                                                             // 489
    ...
    stop: function() {                                                                                             // 491
      this.done = true;                                                                                            // 492
                                                                                                                   // 493
      var rootEntry = this.tryEntries[0];                                                                          // 494
      var rootRecord = rootEntry.completion;                                                                       // 495
      if (rootRecord.type === "throw") {                                                                           // 496
        throw rootRecord.arg;                                                                                      // 497
      }                                                                                                            // 498
                                                                                                                   // 499
      return this.rval;                                                                                            // 500
    },                                                                                                             // 501
                                                                                                                   // 502
    dispatchException: function(exception) {                                                                       // 503
      if (this.done) {                                                                                             // 504
        throw exception;                                                                                           // 505
      }                                                                                                            // 506
                                                                                                                   // 507
      var context = this;                                                                                          // 508
      function handle(loc, caught) {                                                                               // 509
        record.type = "throw";                                                                                     // 510
        record.arg = exception;                                                                                    // 511
        context.next = loc;                                                                                        // 512
        return !!caught;                                                                                           // 513
      }                                                                                                            // 514
                                                                                                                   // 515
      for (var i = this.tryEntries.length - 1; i >= 0; --i) {                                                      // 516
        var entry = this.tryEntries[i];                                                                            // 517
        var record = entry.completion;                                                                             // 518
                                                                                                                   // 519
        if (entry.tryLoc === "root") {                                                                             // 520
          // Exception thrown outside of any try block that could handle                                           // 521
          // it, so set the completion value of the entire function to                                             // 522
          // throw the exception.                                                                                  // 523
          return handle("end");                                                                                    // 524
        }                                                                                                          // 525
                                                                                                                   // 526
        if (entry.tryLoc <= this.prev) {                                                                           // 527
          var hasCatch = hasOwn.call(entry, "catchLoc");                                                           // 528
          var hasFinally = hasOwn.call(entry, "finallyLoc");                                                       // 529
                                                                                                                   // 530
          if (hasCatch && hasFinally) {                                                                            // 531
            if (this.prev < entry.catchLoc) {                                                                      // 532
              return handle(entry.catchLoc, true);                                                                 // 533
            } else if (this.prev < entry.finallyLoc) {                                                             // 534
              return handle(entry.finallyLoc);                                                                     // 535
            }                                                                                                      // 536
                                                                                                                   // 537
          } else if (hasCatch) {                                                                                   // 538
            if (this.prev < entry.catchLoc) {                                                                      // 539
              return handle(entry.catchLoc, true);                                                                 // 540
            }                                                                                                      // 541
                                                                                                                   // 542
          } else if (hasFinally) {                                                                                 // 543
            if (this.prev < entry.finallyLoc) {                                                                    // 544
              return handle(entry.finallyLoc);                                                                     // 545
            }                                                                                                      // 546
                                                                                                                   // 547
          } else {                                                                                                 // 548
            throw new Error("try statement without catch or finally");                                             // 549
          }                                                                                                        // 550
        }                                                                                                          // 551
      }                                                                                                            // 552
    },                                                                                                             // 553
                                                                                                                   // 554
    abrupt: function(type, arg) {                                                                                  // 555
      for (var i = this.tryEntries.length - 1; i >= 0; --i) {                                                      // 556
        var entry = this.tryEntries[i];                                                                            // 557
        if (entry.tryLoc <= this.prev &&                                                                           // 558
            hasOwn.call(entry, "finallyLoc") &&                                                                    // 559
            this.prev < entry.finallyLoc) {                                                                        // 560
          var finallyEntry = entry;                                                                                // 561
          break;                                                                                                   // 562
        }                                                                                                          // 563
      }                                                                                                            // 564
                                                                                                                   // 565
      if (finallyEntry &&                                                                                          // 566
          (type === "break" ||                                                                                     // 567
           type === "continue") &&                                                                                 // 568
          finallyEntry.tryLoc <= arg &&                                                                            // 569
          arg <= finallyEntry.finallyLoc) {                                                                        // 570
        // Ignore the finally entry if control is not jumping to a                                                 // 571
        // location outside the try/catch block.                                                                   // 572
        finallyEntry = null;                                                                                       // 573
      }                                                                                                            // 574
                                                                                                                   // 575
      var record = finallyEntry ? finallyEntry.completion : {};                                                    // 576
      record.type = type;                                                                                          // 577
      record.arg = arg;                                                                                            // 578
                                                                                                                   // 579
      if (finallyEntry) {                                                                                          // 580
        this.next = finallyEntry.finallyLoc;                                                                       // 581
      } else {                                                                                                     // 582
        this.complete(record);                                                                                     // 583
      }                                                                                                            // 584
                                                                                                                   // 585
      return ContinueSentinel;                                                                                     // 586
    },                                                                                                             // 587
                                                                                                                   // 588
    complete: function(record, afterLoc) {                                                                         // 589
      if (record.type === "throw") {                                                                               // 590
        throw record.arg;                                                                                          // 591
      }                                                                                                            // 592
                                                                                                                   // 593
      if (record.type === "break" ||                                                                               // 594
          record.type === "continue") {                                                                            // 595
        this.next = record.arg;                                                                                    // 596
      } else if (record.type === "return") {                                                                       // 597
        this.rval = record.arg;                                                                                    // 598
        this.next = "end";                                                                                         // 599
      } else if (record.type === "normal" && afterLoc) {                                                           // 600
        this.next = afterLoc;                                                                                      // 601
      }                                                                                                            // 602
    },                                                                                                             // 603
                                                                                                                   // 604
    finish: function(finallyLoc) {                                                                                 // 605
      for (var i = this.tryEntries.length - 1; i >= 0; --i) {                                                      // 606
        var entry = this.tryEntries[i];                                                                            // 607
        if (entry.finallyLoc === finallyLoc) {                                                                     // 608
          this.complete(entry.completion, entry.afterLoc);                                                         // 609
          resetTryEntry(entry);                                                                                    // 610
          return ContinueSentinel;                                                                                 // 611
        }                                                                                                          // 612
      }                                                                                                            // 613
    },                                                                                                             // 614
                                                                                                                   // 615
    "catch": function(tryLoc) {                                                                                    // 616
      for (var i = this.tryEntries.length - 1; i >= 0; --i) {                                                      // 617
        var entry = this.tryEntries[i];                                                                            // 618
        if (entry.tryLoc === tryLoc) {                                                                             // 619
          var record = entry.completion;                                                                           // 620
          if (record.type === "throw") {                                                                           // 621
            var thrown = record.arg;                                                                               // 622
            resetTryEntry(entry);                                                                                  // 623
          }                                                                                                        // 624
          return thrown;                                                                                           // 625
        }                                                                                                          // 626
      }                                                                                                            // 627
                                                                                                                   // 628
      // The context.catch method must only be called with a location                                              // 629
      // argument that corresponds to a known catch block.                                                         // 630
      throw new Error("illegal catch attempt");                                                                    // 631
    },                                                                                                             // 632
                                                                                                                   // 633
    delegateYield: function(iterable, resultName, nextLoc) {                                                       // 634
      this.delegate = {                                                                                            // 635
        iterator: values(iterable),                                                                                // 636
        resultName: resultName,                                                                                    // 637
        nextLoc: nextLoc                                                                                           // 638
      };                                                                                                           // 639
                                                                                                                   // 640
      return ContinueSentinel;                                                                                     // 641
    }                                                                                                              // 642
  };                                                                                                               // 643
})(                                                                                                                // 644
  // Among the various tricks for obtaining a reference to the global                                              // 645
  // object, this seems to be the most reliable technique that does not                                            // 646
  // use indirect eval (which violates Content Security Policy).                                                   // 647
  typeof global === "object" ? global :                                                                            // 648
  typeof window === "object" ? window :                                                                            // 649
  typeof self === "object" ? self : this                                                                           // 650
);                                                                                                                 // 651
                                                                                                                   // 652
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
